...
export class SessionRepairService extends EventEmitter {
  ...
  async waitForSession(sessionId: string, timeoutMs = 30000): Promise<SessionScanResult> {
    ...
    if (!filePath) {
      // Session file doesn't exist. Treat as 'missing' so terminal.create can
      // safely fall back to starting a fresh session.
      const missing: SessionScanResult = {
        sessionId,
        filePath: '',
        status: 'missing',
        chainDepth: 0,
      ...
      this.queue.seedResult(sessionId, missing)
      return missing
    }

...
impl FsProbe for FakeFsProbe {
    ...
    fn list_session_files(&self, spec: &ProviderSpec) -> Vec<PathBuf> {
        self.0.lock().unwrap().files.get(&spec.name).cloned().unwrap_or_default()
    }
}
...
impl WatcherFactory for FakeWatcherFactory {
    fn create(&self, label: &'static str) -> Box<dyn Watcher> {
        Box::new(FakeWatcher { state: self.0.clone(), label })
    }
}
...
fn late_root_deterministic() {
    ...
    assert!(
        boot_calls.iter().any(|(label, paths)| *label == "root" && paths.contains(&home)),
        "boot must arm the late-root watcher on the existing ancestor (home): {boot_calls:?}"
    );
    assert!(indexer.snapshot().is_empty(), "no sessions before the subdir appears");
    let boot_events = indexer.drain_events();
    assert!(boot_events.contains(&IndexerEvent::Reconfigured { key: "claude:0".to_string() }));
    ...
    //     transiently fails (the injected close-during-add race). ---
    {
        ...
        st.files.insert("claude".to_string(), vec![session_file.clone()]);
    ...
    assert!(
        events.iter().any(|e| matches!(e, IndexerEvent::WatchArmFailedDegraded { label: "glob", .. })),
    ...
    assert!(indexer.rescan_pending(), "indexer is degraded-but-alive with a pending rescan");

    // The reconfigure still re-armed the root watcher on the now-existing subdir.
    let reconfig_calls = take_watch_calls(&shared);
    assert!(
        reconfig_calls.iter().any(|(label, paths)| *label == "root" && paths.contains(&projects)),
    ...
    assert_eq!(indexer.snapshot(), vec![session_file], "the late session is now indexed");
...
fn backend_error_event_degrades_without_dying() {
    ...
    assert!(events.contains(&IndexerEvent::RescanScheduled), "and a rescan is scheduled: {events:?}");
    assert!(indexer.rescan_pending(), "still alive, degraded with a pending rescan");
}
...
fn happy_path_reconfigure_on_appearance_stays_alive() {
    ...
    let home = tmp.join(".claude");
    let projects = home.join("projects");
    let file = projects.join("hash").join("00000000-0000-4000-8000-000000000002.jsonl");
...
fn late_root_real_notify() {
    ...
    assert!(
        snapshot.iter().any(|p| p.ends_with("00000000-0000-4000-8000-000000000003.jsonl")),

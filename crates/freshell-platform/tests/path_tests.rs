...
use freshell_platform::path::{
    convert_windows_path_to_wsl_path, convert_wsl_drive_path_to_windows_path, detect_user_path_flavor,
    is_linux_path, resolve_launch_cwd, sanitize_user_path_input, win32_resolve, LaunchCwdConversion,
...
fn flavor_detection_matrix() {
    ...
    for w in ["C:", "C:\\", "c:/", "D:\\a", "\\\\server\\share\\x", "\\foo"] {
...
fn win32_resolve_goldens_from_node() {
    let rows: &[(&str, Option<&str>)] = &[
        ...
        ("\\\\wsl.localhost\\Ubuntu", Some("\\\\wsl.localhost\\Ubuntu\\")), // UNC root -> trailing sep
        ("//wsl.localhost/Ubuntu/x", Some("\\\\wsl.localhost\\Ubuntu\\x")),
    ...
    for (input, expected) in rows {
        assert_eq!(win32_resolve(input).as_deref(), *expected, "win32_resolve({input:?})");
    }
}
...
fn wsl_drive_to_windows_custom_mount_prefix() {
    ...
    assert_eq!(convert_wsl_drive_path_to_windows_path("/mnt/c/x", &env), None);
}
...
fn windows_to_wsl_custom_and_root_mount() {
    ...
    assert_eq!(convert_windows_path_to_wsl_path("C:\\", &custom, true).as_deref(), Some("/custom/c"));

    // Empty-prefix (root) mount: `C:\ -> /c`, `D:\a\b -> /d/a/b`.
    let root = MapEnv::new()
        .with("WSL_DISTRO_NAME", "Ubuntu")
        .with("WSL_WINDOWS_SYS32", "/c/Windows/System32");
    assert_eq!(convert_windows_path_to_wsl_path("C:\\", &root, true).as_deref(), Some("/c"));
    assert_eq!(convert_windows_path_to_wsl_path("D:\\a\\b", &root, true).as_deref(), Some("/d/a/b"));
}
...
fn launch_cwd_linux_process_matrix() {
    ...
    for c in ["C:proj", "\\\\srv\\share", "\\rooted", "//wsl.localhost/Ubuntu/x"] {
        assert_eq!(resolve_launch_cwd(Some(c), lp, &env, true).launch_cwd, None, "{c:?}");
    }
...
fn launch_cwd_windows_process_matrix() {
    ...
    assert_eq!(resolve_launch_cwd(Some("/mnt/c/proj"), wp, &MapEnv::new(), false).launch_cwd, None);

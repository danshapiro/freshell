use freshell_runtime_protocol::{
    ...
    LaunchState, RequestId, RuntimeLimits, RuntimeView, SoulId, StopOutcome,
};
...
impl Registry {
    ...
    pub async fn prepare_launch(
        ...
        self.run_blocking(move |mut conn| {
            ...
            let now = now_millis();
            let soul_exists: bool = tx.query_row(
            ...
            if !soul_exists {
                tx.execute(
                    "INSERT INTO souls (soul_id,provider,provider_store_id,native_session_id,creation_seed_ref,desired_state,intent_revision,recovery_state,durability_state,checkpoint_revision,created_at,updated_at) VALUES (?1,?2,?3,NULL,?4,'running',1,'live','unknown',0,?5,?5)",
                    params![input.soul_id.as_str(), input.provider, input.provider_store_id, input.creation_seed_ref, now],
                )?;
            }
...
fn load_owned_handle(
    ...
    let raw: OwnedRow = conn.query_row(
        "SELECT soul_id,launch_nonce,docker_daemon_id,container_id,image_ref,runtime_dir,host_binary_path,immutable_config_digest,requested_limits,launch_state FROM incarnations WHERE incarnation_id=?1",
        params![incarnation_id.as_str()],
...
fn load_inventory(conn: &Connection) -> Result<Vec<RuntimeView>, RegistryError> {
    let mut stmt = conn.prepare("SELECT i.soul_id,i.incarnation_id,i.launch_state,i.cleanup_state,s.intent_revision,i.container_id,i.host_boot_id,i.execution_generation,i.effective_limits FROM incarnations i JOIN souls s ON s.soul_id=i.soul_id ORDER BY i.created_at,i.incarnation_id")?;

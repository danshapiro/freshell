...
- Focused test command: `npm run test:vitest -- run <path>` for `test/unit/**` and `test/e2e/**` (default config); append `--config vitest.server.config.ts` for `test/server/**`.
...
Run: `npm run test:vitest -- run test/unit/server/fresh-agent/codex-adapter.test.ts -t "self-heals to exited"`
...
Run: `npm run test:vitest -- run test/unit/server/fresh-agent/codex-adapter.test.ts`
...
Run: `npm run test:vitest -- run test/unit/server/fresh-agent/sdk-events.test.ts -t "freshAgent.turn.waiting"`
...
Run: `npm run test:vitest -- run test/unit/server/fresh-agent/sdk-events.test.ts`
Expected: PASS.
...
Run: `npm run test:vitest -- run test/unit/server/sdk-bridge.test.ts -t "sdk.turn.waiting"`
...
Run: `npm run test:vitest -- run test/unit/server/sdk-bridge.test.ts`

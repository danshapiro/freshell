...
List all creation sources: browser picker, split, history resume, WS create/attach, REST tab/split, MCP new-tab/split, restore/reconcile, auto-resume, and provider-created managed child agents. Each must invoke the same supervisor operation. Add structural tests and a manually reviewed spawn-site inventory so no production path bypasses ownership or limits.
...
**Live-provider policy:** fixture protocols are required for deterministic impossible-to-request faults, but cannot replace P3-G01–03's real coding-agent proof. Use actual enabled Kilroy/Amplifier/plugin runtimes; a missing runtime or test credential blocks release of that enabled mode. Explicitly disabled modes are recorded as out of scope, never silently skipped inside an enabled matrix.

...
- [Added after focused-episode-5 round 3's review (retire-on-kill round 4):] four wrong-durable-truth shapes around the claim lifecycle are closed — (a) a kill landing while an attach/resume awaited the provider could be undone by the earlier claim's commit (the commit cleared the newer close's tombstone and revived the row, reopening an orphan for a pane the user closed): every claim lane now snapshots the identity's durable dead-state at claim start, and the ledger's commit is CONDITIONAL — an advanced dead-state refuses the commit with no side effects (no clear, no revive, no alias consumption, no registration), and the lane tears the just-built session down; (b) the claim's tombstone-clear and row-revive were two awaited, log-and-continue writes a crash could split either way: the commit is now ONE ledger transition — the revived row write lands first and the tombstone delete second, so the only reachable mid-crash state (the Bound row outliving its stale tombstone) reads as COMMITTED everywhere (never dominant, never suppressing, pruned by any sweep); (c) a kill inside the eviction's two-phase window (cli_index released before the alias-tombstone record) could resolve NEITHER map and retire only the placeholder: demote+remove is now one critical section (cli_index held across the tombstone record, matching the kill consult's lock order); (d) the 6h kill-tombstone TTL could outrun the crash durability it protected (tombstone written, retire lost, restart hours later — the Bound row resurfaced): dominance is now the pair-relative classification (close-as-new-as-or-newer-than a Bound row's liveness), so a tombstone paired with a still-Bound row never expires — the sweeps converge the row first and prune the pair only then, and the recovery offer applies the same TTL-free dominance. [Amended after focused-episode-5 round 4's review (retire-on-kill round 5):] the round-3 residual named here is CLOSED — see the next bullet.
- [Added after focused-episode-5 round 4's review (retire-on-kill round 5):] the last four lifecycle gaps in the kill-vs-claim state machine are closed under two design rules — (1) the durable close (kill tombstone + row retire, one ledger write, plus the opencode pending-marker delete) is recorded BEFORE any teardown/settlement await, so a server crash inside a kill's sidecar/turn settlement can never lose the close (the codex and opencode kill handlers ran their settlement awaits first; the claude kill's consumer join now also sits after its retire); and (2) kill always wins vs an in-flight claim — a kill QUEUED behind a successful claim commit (the commit's dead-state decision pre-dated it, the finding-1 interleaving) landed between the commit and the registration, leaving a live session over a Closed row: every claim lane now runs a POST-REGISTRATION dead-state re-check against its claim-start snapshot (an advanced fence ⇒ the registered session is a late orphan, torn down with the row left Retired), every kill lane retires the durable row BEFORE its session-map removal (so the removal still finds a session the claim registered mid-kill), and the claude kill additionally sweeps orphans a late-retire order produced (a claim whose own commit cleared the fence afterwards is a genuine reopen and is spared). Also closed: (3) the claude alias tombstones' 6h TTL / 1024-cap could discard the placeholder→durable mapping under a long-lived pane, so a days-later close retired only the placeholder — alias records now live as long as the row they resolve to (TTL expiry and capacity eviction both consult the row's raw Bound state through a new sync `row_is_bound` sink read; a wholly bound set may exceed the cap); (4) every provider treated a `commit_claim` `Err` as committed and registered anyway, but the ledger's commit leaves the close untouched on error — all three providers now treat the error like a refusal (tear down; kill wins).
- [Added after focused-episode-5 round 5's review (retire-on-kill round 6):] three cross-cutting durability/concurrency gaps closed — (a) the opencode kill lane held the sessions-map lock while awaiting the per-session lock (and `handle_send` takes the same pair the other way across its cold-start materialization): closing a pane during its first `create_session` deadlocked, the close never reached its durable retire, and the held map wedged every other opencode operation — the file now has ONE lock rule (never hold `sessions` across a per-session lock acquisition; clone the Arc out, drop the map guard, then take the session lock) on all three sites (the kill's capture phase, its teardown block, and the resume-refusal teardown's `if let` scrutinee shape); (b) the claude placeholder→durable alias mapping still lived only in process memory, so a post-restart close by the bare placeholder fenced/retired nothing durable and the reconnect's slow resume then committed the row Bound for a pane already gone — the mapping is now durable in the ledger's `alias-tombstones/` subtree (minted at registration/adoption, re-stamped at demotion, lifetime tied to the row it resolves to), the kill's alias consult folds the persisted records in, and the claim commit consults the close fence under the one-shot pane seat it rides (a fence recorded under the placeholder blocks the commit exactly like one recorded under the durable id — existence-based for seats, since a placeholder is never genuinely re-claimed): a phantom post-restart attach is refused before registration and the killed row stays Retired; (c) the kill's post-close sweep was check-then-act (a claim committing between its fence check and its session-map removal got its genuine reopen destroyed and its revived row left Bound — a recovery ghost): the removal and the fence re-verify are now ONE sessions-map critical section (a mid-sweep commit spares the session; a still-standing fence means the destroyed claim's row is re-retired idempotently).

...
- [ ] **TERM-02 — Use the managed Codex app-server path.** Launch Codex with the managed remote connection, disable unsupported app behavior, capture durable thread identity, and retain the lifecycle/ownership contract.
  ...
  - PARTIAL (2026-07-18, demoted from [x] on review): `test/e2e-browser/specs/restore-matrix.spec.ts` :: "FreshCodex targets the same durable thread with no duplicate conversation after a full server restart" (scenario 5, commit 94a3ca94 fix) proves durable thread identity, restart resume, and no duplicate conversation — green both projects. MISSING: the "disable unsupported app behavior" clause has no assertion anywhere in the cited evidence.
...
- [ ] **TERM-18 — Recover provider process loss.** Clear activity, mark the pane exited, preserve the durable identity, and lazily restart/recover where legacy does; never chime on a crash.
  ...
  - PARTIAL (2026-07-18, demoted from [x] on review): `test/e2e-browser/specs/restore-matrix.spec.ts` :: "a crashed Codex provider process is recovered mid-turn with no chime, and the same durable session continues" (scenario 6) proves crash detection ("session has ended" banner), no chime (no `freshAgent.turn.complete`), durable id preservation, and recovery — green both projects. MISSING: the "clear activity"/"assert blue clears" clause — no busy-indicator assertion exists in the cited scenario.

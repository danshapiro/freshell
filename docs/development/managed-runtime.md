...
Managed providers strip web-owned MCP/rebind machinery. Managed Claude omits the legacy temporary `--mcp-config`; managed OpenCode omits the host rebind plugin, project-local Freshell MCP mutation, host-side SQLite locator, and web-side loopback SSE lane. OpenCode runs one pinned provider runtime per soul on private loopback, with provider-native identity learned inside the enclosure. The managed fresh-agent path likewise owns Claude/Kilroy, Codex, and OpenCode provider transports in the session host; the Rust web process is only a gateway and event projection surface. These managed paths can therefore survive web-process replacement without transferring provider-process ownership back to the web tier. Non-managed Rust callers, Codex terminal sidecars, Node-server routes, and the durable capability-scoped tool-router work remain separate migration surfaces; none may be counted as managed merely because a nearby provider adapter exists.

Provider topology is not Freshell lifecycle topology. A provider-native subagent or child thread remains inside the parent soul and consumes that soul's cgroup/resource reservation. A provider-native conversation fork is an in-soul session transition: the host asks the existing provider runtime to branch, verifies the old provider session is retired, updates the durable actor identity, and the supervisor atomically replaces the parent writer claim with the child identity for the same soul/incarnation. The old parent resume/checkpoint proof is invalidated (`live_only` / `materializing`) until the child is re-probed. All current views of that managed soul converge to the new native branch; no `Launch` or soul `Stop` is generated by the fork itself. If the host dies after provider acceptance but before durable completion, the persisted command becomes `AMBIGUOUS` and is never automatically replayed. A separately managed Freshell agent is created only by an explicit product operation that requests independent lifecycle/resource ownership.

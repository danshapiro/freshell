...
- Pre-worktree green-base checks (and any broad gate intended to validate `origin/main` itself, as opposed to a branch under test) go through `scripts/base-gate.sh` (e.g. `scripts/base-gate.sh test`), which runs the command from a clean scratch worktree at `origin/main`. The main checkout accumulates untracked litter, so its cloud test image is tagged by content (`<sha>-dirty-<hash>`) and rebuilt (~13 min cold) whenever that litter changes, whereas a clean worktree uses the commit tag, built once per commit and shared by every later run. How cloud test images are named, reused, pinned to the exact tree, deduplicated, and cleaned up: [docs/development/cloud-test-images.md](docs/development/cloud-test-images.md).

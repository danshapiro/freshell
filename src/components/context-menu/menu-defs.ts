...
export type MenuActions = {
  ...
  copyFreshAgentCodeBlock: (clickTarget: HTMLElement | null) => void
  copyFreshAgentToolInput: (clickTarget: HTMLElement | null) => void
  copyFreshAgentToolOutput: (clickTarget: HTMLElement | null) => void
  copyFreshAgentDiffNew: (clickTarget: HTMLElement | null) => void
  copyFreshAgentDiffOld: (clickTarget: HTMLElement | null) => void
  copyFreshAgentFilePath: (clickTarget: HTMLElement | null) => void
...
export type MenuBuildContext = {
  ...
  /** Whether the server advertised `agentRestartV1` on the ready frame.
   * Withheld on platforms whose persisted replacement-fence recovery cannot
   * succeed — the menu then degrades Restart to Refresh (fail closed). */
  agentRestartSupported: boolean
...
function buildPaneLifecycleItem(
  ...
  if (content && agentRestartSupported && resolveAgentRestartTarget(content)) {
    return {
      type: 'item',
      id: 'restart-pane',
      label: 'Restart pane',
...
export function buildMenuItems(target: ContextTarget, ctx: MenuBuildContext): MenuItem[] {
  ...
  if (target.kind === 'global') {
    ...
    return [
      ...
      { type: 'item', id: 'reopen-tab', label: 'Reopen closed tab', shortcut: 'Alt+H', onSelect: actions.reopenClosedTab },
  ...
  if (target.kind === 'pane') {
    ...
    return [
      ...paneResumeMenuItem,
      ...reopenPaneAsItem,
      buildPaneLifecycleItem(target.tabId, target.paneId, paneContent, ctx.agentRestartSupported, actions),
  ...
  if (target.kind === 'fresh-agent') {
    ...
    if (codeBlock) {
      items.push(
        { type: 'separator', id: 'fc-code-sep' },
        {
          ...
          onSelect: () => actions.copyFreshAgentCodeBlock(codeBlock),
        },
      )
    }
    ...
    if (toolInput) {
      ...
      items.push(
        { type: 'separator', id: 'fc-tool-input-sep' },
        {
          ...
          onSelect: () => actions.copyFreshAgentToolInput(toolInput),
        },
      )
    }
    ...
    if (toolOutput) {
      ...
      items.push({
        ...
        id: 'fc-copy-output',
        label: 'Copy output',
        onSelect: () => actions.copyFreshAgentToolOutput(toolOutput),
      })
    }
    ...
    if (diffView) {
      ...
      items.push(
        { type: 'separator', id: 'fc-diff-sep' },
        {
          ...
          onSelect: () => actions.copyFreshAgentDiffNew(diffView),
        },
        {
          type: 'item',
          id: 'fc-copy-old-version',
          label: 'Copy old version',
          onSelect: () => actions.copyFreshAgentDiffOld(diffView),
        },
      )
      if (filePath) {
        items.push({
          ...
          onSelect: () => actions.copyFreshAgentFilePath(diffView),
        })
      }
    }

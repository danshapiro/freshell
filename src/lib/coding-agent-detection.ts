...
/**
 * A pane runs a CLI coding agent iff it is a fresh-agent pane, a (legacy)
 * agent-chat pane, or a terminal in a non-shell coding mode (claude/codex/
...
export function isCodingAgentContent(content: PaneContent | undefined | null): boolean {
  ...
  if (content.kind === 'fresh-agent' || content.kind === 'agent-chat') return true

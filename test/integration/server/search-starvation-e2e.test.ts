...
/**
 * E2E test: deep session search must not starve other foreground requests.
 *
 * The default readModelScheduler is shared by session-directory, shell-bootstrap,
 * and terminal-directory routers.  With foregroundConcurrency = 1, a long-running
 * userMessages/fullText file search blocks all other visible/critical requests,
 * making the server appear dead (the user can't open tabs, fetch terminals, etc.).
 *
 * These tests mount real Express routers on a real HTTP server, wired through the
 * real defaultReadModelScheduler, and exercise the full HTTP round-trip.
 */
...
describe('deep search does not starve other routes (e2e)', () => {
  ...
  async function createServer(projects: ProjectGroup[]) {
    ...
    // Wire BOTH routers through the real defaultReadModelScheduler (no override).
  ...
  it('bootstrap request completes while a foreground slot is occupied', async () => {
    // Set up a minimal server (projects don't matter for this test).
    await createServer([])

    // Occupy a foreground slot by scheduling a slow visible task directly
    // on the same defaultReadModelScheduler the routers use.
    ...
    // Allow the slow task to start.
    await new Promise((r) => setTimeout(r, 10))

    // Fire bootstrap through HTTP — it uses the same scheduler (critical lane).
    // With foregroundConcurrency >= 2, this should complete immediately.
    // With foregroundConcurrency = 1, it would block behind the slow task.
    ...
    // Clean up: release the slow task.
    slowTask.resolve()
    await slowTaskPromise
  })

  it('two concurrent session-directory requests both complete', async () => {
    // Two visible-lane requests running concurrently proves the
    // scheduler allows multiple foreground tasks.
    ...
    // Fire two deep searches concurrently — both use the visible lane.
  ...
  it('fullText tier finds assistant message matches', async () => {
    ...
    // Append an assistant message with a unique keyword.

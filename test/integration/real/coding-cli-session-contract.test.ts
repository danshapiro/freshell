...
describe.sequential('coding cli real provider session contract', () => {
  ...
  if (!codexBinary.resolvedPath) {
    it.skip('skips Codex contract because executable codex is unavailable', () => {})
  } else {
    describe.sequential('codex', () => {
      it('matches the recorded binary path and version, and uses the expected remote bootstrap forms', async () => {
        expect(codexBinary.resolvedPath).toBe(note.providers.codex.resolvedPath)
        ...
        try {
          ...
          const freshEvents = await captureCodexBootstrapEvents(workspace, codexBinary.resolvedPath)
      ...
      it('stays live-only until the durable artifact exists, then restores via the artifact id', async () => {
        ...
        try {
          ...
          const liveOnlyAppServer = await startCodexAppServer(workspace, codexBinary.resolvedPath!)
          ...
          const createAppServer = await startCodexAppServer(workspace, codexBinary.resolvedPath!)
          ...
          const resumeBootstrapEvents = await captureCodexResumeBootstrapEvents(
            workspace,
            codexBinary.resolvedPath!,
            resumeId,
          )
          expect(resumeBootstrapEvents).toEqual(note.providers.codex.remoteResumeBootstrapEventsBeforeUserTurn)

          const resumedAppServer = await startCodexAppServer(workspace, codexBinary.resolvedPath!)
  ...
  if (!claudeBinary.resolvedPath) {
    it.skip('skips Claude contract because executable claude is unavailable', () => {})
  } else {
    describe.sequential('claude', () => {
      it('matches the recorded binary path and version', async () => {
        expect(claudeBinary.resolvedPath).toBe(note.providers.claude.resolvedPath)
  ...
  if (!opencodeBinary.resolvedPath) {
    it.skip('skips OpenCode contract because executable opencode is unavailable', () => {})
  } else {
    describe.sequential('opencode', () => {
      it('matches the recorded binary path and version', async () => {
        expect(opencodeBinary.resolvedPath).toBe(note.providers.opencode.resolvedPath)

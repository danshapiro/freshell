  ...
  addTerminal(
    ...
    const terminal = {
      terminalId:
        ...
        `term_${Date.now()}_${Math.random().toString(16).slice(2, 8)}`,
...
describe("Terminals API", () => {
  ...
  describe("GET /api/terminals", () => {
    it("returns 401 when no auth token provided", async () => {
      const response = await request(app).get("/api/terminals").expect(401);
    ...
    it("lists all terminals with required fields", async () => {
      ...
      const firstTerminal = response.body.find(
        (t: any) => t.terminalId === "term_123",
      );
      ...
      const secondTerminal = response.body.find(
        (t: any) => t.terminalId === "term_456",
      );
    ...
    it("includes the last emitted terminal line", async () => {
      ...
      const item = response.body.find(
        (t: any) => t.terminalId === "term_last_line",
      );
    ...
    it("includes all terminal modes: shell, claude, codex", async () => {
      registry.addTerminal({
        terminalId: "shell_term",
        title: "Shell",
        mode: "shell",
      });
      registry.addTerminal({
        terminalId: "claude_term",
        title: "Claude CLI",
        mode: "claude",
      });
      registry.addTerminal({
        terminalId: "codex_term",
        title: "Codex CLI",
        mode: "codex",
      });
    ...
    it("keeps scrollback and search on separate server-owned routes", async () => {
      ...
      expect(search.body).toEqual({
        matches: [{ line: 0, column: 0, text: "alpha" }],
        nextCursor: "1",
      });
    });
  });
  ...
  describe("PATCH /api/terminals/:terminalId", () => {
    ...
    it(
      "updates terminal title override",
      async () => {

...
vi.mock('@/components/Sidebar', () => ({
  default: ({ onNavigate }: { onNavigate: (view: 'terminal' | 'sessions' | 'overview' | 'settings') => void }) => (
    <div data-testid="mock-sidebar">
      <button type="button" onClick={() => onNavigate('sessions')}>Go sessions</button>
      <button type="button" onClick={() => onNavigate('settings')}>Go settings</button>
    </div>
  ),
  AppView: {} as any,
}))
...
describe('App lazy-loaded views', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    localStorage.clear()
  ...
  it('loads HistoryView and SettingsView on demand', async () => {

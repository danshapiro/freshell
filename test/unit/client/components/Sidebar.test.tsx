...
describe('Sidebar Component - Session-Centric Display', () => {
  ...
  describe('sidebar click opens pane', () => {
    ...
    it('preserves exact local session identity when splitting a running session into the current tab', async () => {
      const targetSessionId = sessionId('running-session-to-split')
          ...
          sessions: [
            {
              ...
              projectPath: '/home/user/project',
              lastActivityAt: Date.now(),
              title: 'Running session to split',
              cwd: '/home/user/project',
            },
          ],
        },
      ...
      const sessionButton = screen.getByText('Running session to split').closest('button')
      fireEvent.click(sessionButton!)
      ...
      if (localPane?.type === 'leaf' && localPane.content.kind === 'terminal') {
        expect(localPane.content).toMatchObject({
          terminalId: 'term-running',
          status: 'running',
          sessionRef: {
            provider: 'codex',

...
describe('TerminalView lifecycle updates', () => {
  ...
  it('surfaces a visible notice when buffered input times out un-anchored', async () => {
    ...
    try {
      ...
      act(() => {
        ...
        // writeLocalXtermNotice goes through TerminalWriteQueue, which submits
        // on the next animation frame. Vitest 5 no longer drains an rAF that
        // was scheduled by the last timer inside the same advance call.
  ...
  it('re-writes the loss notice after the pane re-anchors (term.clear survival)', async () => {
    ...
    try {
      ...
      act(() => {
        vi.advanceTimersByTime(30_001) // timeout -> notice enqueue
        vi.advanceTimersByTime(20) // TerminalWriteQueue -> next animation frame
      })
      ...
      act(() => {
        ...
        // The post-anchor notice is also serialized through TerminalWriteQueue.
        vi.advanceTimersByTime(20)
      })

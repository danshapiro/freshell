...
describe('migrateLegacyEnvFile', () => {
  const anchor = '/srv/pinned/.env'
  const legacy = '/home/user/.env'

  function makeDeps(files: Record<string, string>, opts: { copyFails?: boolean } = {}) {
    ...
    const deps = {
      ...
      copyFileSync: (src: string, dest: string) => {
        ...
        fsMap.set(dest, fsMap.get(src)!)
        calls.copied.push([src, dest])
      },
      log: (m: string) => calls.logs.push(m),
      error: (m: string) => calls.errors.push(m),
    }
    return { deps, calls, read: (p: string) => fsMap.get(p) }
  }

  it('is a no-op when the target already exists', () => {
    const { deps, calls } = makeDeps({ [anchor]: 'AUTH_TOKEN=x', [legacy]: 'AUTH_TOKEN=old' })
  ...
  it('copies a legacy file whose AUTH_TOKEN first line is a comment (the real generated layout)', () => {
    const { deps, calls, read } = makeDeps({
      [legacy]: '# Auto-generated by Freshell first-run setup\nAUTH_TOKEN=real-token\nPORT=3001\n',
    })
    expect(migrateLegacyEnvFile(anchor, legacy, deps)).toBe(true)
    expect(read(anchor)).toContain('AUTH_TOKEN=real-token')
    expect(calls.copied).toEqual([[legacy, anchor]])
  })
  ...
  it('is a no-op when anchor and legacy are the same path', () => {
    const { deps, calls } = makeDeps({ '/home/user/.env': 'AUTH_TOKEN=x' })
    expect(migrateLegacyEnvFile('/home/user/.env', '/home/user/.env', deps)).toBe(false)
    expect(calls.copied).toEqual([])
  })

  it('copies the legacy .env into the anchor (never moves) and logs once', () => {
    const { deps, calls, read } = makeDeps({ [legacy]: 'AUTH_TOKEN=old' })
    expect(migrateLegacyEnvFile(anchor, legacy, deps)).toBe(true)
    expect(calls.copied).toEqual([[legacy, anchor]])
    expect(read(anchor)).toBe('AUTH_TOKEN=old') // target materialized
    expect(read(legacy)).toBe('AUTH_TOKEN=old') // source intact
    expect(calls.logs).toHaveLength(1)
  })

  it('reports and returns false when the copy fails', () => {
    const { deps, calls } = makeDeps({ [legacy]: 'AUTH_TOKEN=old' }, { copyFails: true })
    expect(migrateLegacyEnvFile(anchor, legacy, deps)).toBe(false)
    expect(calls.errors).toHaveLength(1)
    expect(calls.errors[0]).toContain('EACCES')
  })
})

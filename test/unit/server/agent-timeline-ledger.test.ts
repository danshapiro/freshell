...
describe('restore ledger manager', () => {
  it('returns typed live-only, merged, and durable-only restore outcomes while upgrading aliases in place', async () => {
    ...
    const durableOnly = await manager.resolve('00000000-0000-4000-8000-000000000123')
    expect(durableOnly).toMatchObject({
      kind: 'resolved',
      readiness: 'durable_only',
      liveSessionId: undefined,
      timelineSessionId: '00000000-0000-4000-8000-000000000123',
    })
    if (durableOnly.kind !== 'resolved') throw new Error('expected resolved')
    expect(durableOnly.turns.map((turn) => turn.messageId)).toEqual([
      'durable-user-1',
      'durable-assistant-1',
    ])
  })
  ...
  it('removes unrecoverable live aliases so stale in-memory authority cannot outlive the session', async () => {
    ...
    expect(afterTeardown).toEqual({ kind: 'missing', queryId: 'named-only' })
  })
})

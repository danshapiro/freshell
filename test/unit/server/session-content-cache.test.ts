...
describe('SessionContentCache', () => {
  ...
  it('cache hit: second get() returns cached messages without re-reading file', async () => {
    ...
    // readFile should not have been called for the second get (stat is OK)
  ...
  it('cache miss on mtime change: re-reads file after modification', async () => {
    ...
    // Wait for mtime resolution (filesystem may have 1s or 1ms granularity)
  ...
  it('LRU eviction: oldest entries evicted when over budget', async () => {
    // Create a small-budget cache
    const smallCache = new SessionContentCache({ maxBytes: 2048 })

    // Create 3 files, each ~1KB of JSONL content
    const files: string[] = []
    for (let i = 0; i < 3; i++) {
      const filePath = path.join(tmpDir, `session-${i}.jsonl`)
      // Each message JSON line is roughly 100-200 bytes, create enough to be ~1KB
    ...
    // Load all 3 files - should trigger eviction
    for (const f of files) {
      await smallCache.get(f)
    }

    // With 2048 byte budget and ~1KB per file, we should have evicted at least one
  ...
  it('size tracking: totalBytes updated on insert and eviction', async () => {
    ...
    // Create a cache with a budget that fits exactly one file, then add a second
    // to trigger eviction. Each minimal JSONL line is ~100 chars + 512 overhead = ~700 bytes.
    ...
    // After inserting the second, the first should have been evicted
    expect(smallCache.stats().entries).toBeLessThanOrEqual(2)
    // totalBytes decreased or stayed bounded
    expect(smallCache.stats().totalBytes).toBeLessThanOrEqual(1500 * 1.5) // allow one entry overshoot
  })
  ...
  it('invalidate() removes specific entry', async () => {
    ...
    // After invalidation, next get should re-read the file
    const readFileSpy = vi.spyOn(fsp, 'readFile')
    await cache.get(filePath)
  ...
  it('malformed JSONL caches partial result', async () => {
    ...
    // Second get should return the same cached result without re-reading
    const readFileSpy = vi.spyOn(fsp, 'readFile')
    const second = await cache.get(filePath)
    expect(second).toHaveLength(2)
    expect(readFileSpy).not.toHaveBeenCalled()

 ...
 *
 * Boots ONE isolated Rust server, negotiates `terminalOutputBatchV1`, and captures
 ...
 *
 * The committed output is frozen Rust-baseline provenance. This utility is
 * retained only for an explicitly reviewed fixture migration and always
...
async function main(): Promise<void> {
  const cols = 120
  const rows = 30
  const server = await startExternalServer({ provider: 'oracle-t1-batch-golden-gen' })

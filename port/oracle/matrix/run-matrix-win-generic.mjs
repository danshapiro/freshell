...
const PREFIX = process.env.MATRIX_PREFIX || 'sbp9-win-chrome'
...
// Windows PATH import is intentionally disabled on this host (HANDOFF SS0) --
// every WSL->Windows interop call MUST use an explicit absolute executable path.
const WIN_SYSTEM32 = '/mnt/c/Windows/System32'
const WIN_CMD = `${WIN_SYSTEM32}/cmd.exe`
const WIN_NETSTAT = `${WIN_SYSTEM32}/netstat.exe`
const WIN_POWERSHELL = `${WIN_SYSTEM32}/WindowsPowerShell/v1.0/powershell.exe`
...
const WIN_TEMP = sh(`"${WIN_CMD}" /d /c "echo %LOCALAPPDATA%\\Temp"`)
...
function netstatPidsOnPort(port) {
  try {
    const out = execSync(`"${WIN_NETSTAT}" -ano`, { encoding: 'utf8' })
    const pids = new Set()
...
async function bootServer() {
  ...
  serverChild = spawn(WIN_CMD, ['/d', '/c', launch], { detached: true, stdio: ['ignore', 'pipe', 'pipe'] })
...
async function runTerminalKind(page, kind) {
  ...
  const content = {
    ...
    createRequestId: `${PREFIX}-${kind.key}-${rand()}`, status: 'creating', initialCwd: WORKSPACE_WIN,
  }
...
async function runBrowserKind(page, kind) {
  const { tabId, paneId } = await openPaneInNewTab(page)
  await dispatchContent(page, tabId, paneId, { kind: 'browser', browserInstanceId: `${PREFIX}-${rand()}`, url: '', devToolsOpen: false })
  const paneSel = `[data-pane-id="${paneId}"]`

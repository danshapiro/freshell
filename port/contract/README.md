...
| `ws-server-messages.schema.json` | JSON Schema for **every server→client message shape** (all 53), keyed by `type` discriminant, plus `wsProtocolVersion`. Synthesized from the `ServerMessage` union via the TypeScript type checker — the **outbound shape contract** for the oracle. |
...
| `crates/freshell-extensions/fixtures/manifest-oracle.json` | Frozen migration provenance consumed by `cargo test -p freshell-extensions`; it is not regenerated by the active contract tooling. |
...
- **TypeScript side:** `shared/ws-protocol.ts` is the *authoring* source.
...
- **Rust oracle:** validates real captured traffic from the owned Rust server
  against these schemas, and uses `ws-message-inventory.json` as the
...
- **Schema bundle:** each exported Zod schema is converted with zod 4's native
  ...
  *structurally* (every exported `ZodType`), not by a `*Schema` name pattern, so
  ...
  dropped. As of this writing all 65 exported schemas convert natively.
